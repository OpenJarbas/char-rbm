...
def pickle_name(args, parser):
    ...
    fname += '_'
    for arg in ['tag', 'batch_size', 'n_hidden', 'softmax', 'learning_rate_backoff', 'preserve_case', 'epochs',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--binomial', action='store_true',
                        help='Use the binomial text codec (for comma-separated two-part names)')
    ...
    parser.add_argument('-e', '--epochs', dest='epochs', default=5, type=int,
                        help="Number of times to cycle through the training data")
    parser.add_argument('--left', action='store_true',
                        help='Pad strings shorter than maxlen from the left rather than the right.')
